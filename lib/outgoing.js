...
Outgoing.prototype.calculateImpliedVolatility = function (reqId, contract, optionPrice, underPrice) {
  if (this._controller.assertVersion(C.MIN_SERVER_VER.REQ_CALC_IMPLIED_VOLAT, 'It does not support calculate implied volatility requests')) return;

  if (!_.isEmpty(contract.tradingClass)) {
    if (this._controller.assertVersion(C.MIN_SERVER_VER.TRADING_CLASS, 'It does not support tradingClass parameter in calculateImpliedVolatility')) return;
  }
...
Outgoing.prototype.calculateOptionPrice = function (reqId, contract, volatility, underPrice) {
  if (this._controller.assertVersion(C.MIN_SERVER_VER.REQ_CALC_OPTION_PRICE, 'It does not support calculate option price requests')) return;

  if (!_.isEmpty(contract.tradingClass)) {
    if (this._controller.assertVersion(C.MIN_SERVER_VER.TRADING_CLASS, 'It does not support tradingClass parameter in calculateOptionPrice')) return;
  }
...
Outgoing.prototype.cancelAccountSummary = function (reqId) {
  if (this._controller.assertVersion(C.MIN_SERVER_VER.ACCT_SUMMARY, 'It does not support account summary cancellation')) return;
...
Outgoing.prototype.cancelCalculateImpliedVolatility = function (reqId) {
  if (this._controller.assertVersion(C.MIN_SERVER_VER.CANCEL_CALC_IMPLIED_VOLAT, 'It does not support calculate implied volatility cancellation')) return;
...
Outgoing.prototype.cancelCalculateOptionPrice = function (reqId) {
  if (this._controller.assertVersion(C.MIN_SERVER_VER.CANCEL_CALC_OPTION_PRICE, 'It does not support calculate option price cancellation')) return;
...
Outgoing.prototype.cancelFundamentalData = function (reqId) {
  if (this._controller.assertVersion(C.MIN_SERVER_VER.FUNDAMENTAL_DATA, 'It does not support fundamental data requests')) return;
...
Outgoing.prototype.cancelHistoricalData = function (tickerId) {
  if (this._controller.assertVersion(24, 'It does not support historical data query cancellation')) return;
...
Outgoing.prototype.cancelMktDepth = function (tickerId) {
  if (this._controller.assertVersion(6, 'This feature is not available for this versions of TWS')) return;
...
Outgoing.prototype.cancelPositions = function () {
  if (this._controller.assertVersion(C.MIN_SERVER_VER.ACCT_SUMMARY, 'It does not support position cancellation')) return;
...
Outgoing.prototype.cancelRealTimeBars = function (tickerId) {
  if (this._controller.assertVersion(C.MIN_SERVER_VER.REAL_TIME_BARS, 'It does not support realtime bar data query cancellation')) return;
...
Outgoing.prototype.cancelScannerSubscription = function (tickerId) {
  if (this._controller.assertVersion(24, 'It does not support API scanner subscription')) return;
...
Outgoing.prototype.exerciseOptions = function (tickerId, contract, exerciseAction, exerciseQuantity,
  ...
  if (this._controller.assertVersion(21, 'It does not support options exercise from the API')) return;

  if (!_.isEmpty(contract.tradingClass) || contract.conId > 0) {
    if (this._controller.assertVersion(C.MIN_SERVER_VER.TRADING_CLASS, 'It does not support conId and tradingClass parameters in exerciseOptions')) return;
  }
...
Outgoing.prototype.placeOrder = function (id, contract, order) {
  ...
  if (this._controller.assertVersion(C.MIN_SERVER_VER.SCALE_ORDERS, 'It does not support Scale orders')) return;
  }
  ...
  if (order.whatIf) {
    if (this._controller.assertVersion(C.MIN_SERVER_VER.WHAT_IF_ORDERS, 'It does not support what-if orders')) return;
  }

  if (contract.underComp) {
    if (this._controller.assertVersion(C.MIN_SERVER_VER.UNDER_COMP, 'It does not support delta-neutral orders')) return;
  }

  if (order.scaleSubsLevelSize !== Number.MAX_VALUE) {
    if (this._controller.assertVersion(C.MIN_SERVER_VER.SCALE_ORDERS2, 'It does not support Subsequent Level Size for Scale orders')) return;
  }

  if (!_.isEmpty(order.algoStrategy)) {
    if (this._controller.assertVersion(C.MIN_SERVER_VER.ALGO_ORDERS, 'It does not support algo orders')) return;
  }

  if (order.notHeld) {
    if (this._controller.assertVersion(C.MIN_SERVER_VER.NOT_HELD, 'It does not support notHeld parameter')) return;
  }

  if (!_.isEmpty(contract.secIdType) || !_.isEmpty(contract.secId)) {
    if (this._controller.assertVersion(C.MIN_SERVER_VER.SEC_ID_TYPE, 'It does not support secIdType and secId parameters')) return;
  }

  if (contract.conId > 0) {
    if (this._controller.assertVersion(C.MIN_SERVER_VER.PLACE_ORDER_CONID, 'It does not support conId parameter')) return;
  }

  if (order.exemptCode !== -1) {
    if (this._controller.assertVersion(C.MIN_SERVER_VER.SSHORTX, 'It does not support exemptCode parameter')) return;
  }
  ...
  if (!_.isEmpty(order.hedgeType)) {
    if (this._controller.assertVersion(C.MIN_SERVER_VER.HEDGE_ORDERS, 'It does not support hedge orders')) return;
  }

  if (order.optOutSmartRouting) {
    if (this._controller.assertVersion(C.MIN_SERVER_VER.OPT_OUT_SMART_ROUTING, 'It does not support optOutSmartRouting parameter')) return;
  }
  ...
  if (order.deltaNeutralConId > 0 ||
    ...
    if (this._controller.assertVersion(C.MIN_SERVER_VER.DELTA_NEUTRAL_CONID, 'It does not support deltaNeutral parameters: ConId, SettlingFirm, ClearingAccount, ClearingIntent')) return;
  }
  ...
  if (!_.isEmpty(order.deltaNeutralOpenClose) ||
    ...
    if (this._controller.assertVersion(C.MIN_SERVER_VER.DELTA_NEUTRAL_OPEN_CLOSE, 'It does not support deltaNeutral parameters: OpenClose, ShortSale, ShortSaleSlot, DesignatedLocation')) return;
  }
  ...
  if (order.scalePriceIncrement > 0 && order.scalePriceIncrement !== Number.MAX_VALUE) {
    ...
    if (this._controller.assertVersion(C.MIN_SERVER_VER.SCALE_ORDERS, 'It does not support Scale order parameters: PriceAdjustValue, PriceAdjustInterval, ProfitOffset, AutoReset, InitPosition, InitFillQty and RandomPercent')) return;
    }
  }
  ...
  if (order.trailingPercent !== Number.MAX_VALUE) {
    if (this._controller.assertVersion(C.MIN_SERVER_VER.TRAILING_PERCENT, 'It does not support trailing percent parameter')) return;
  }

  if (!_.isEmpty(contract.tradingClass)) {
    if (this._controller.assertVersion(C.MIN_SERVER_VER.TRADING_CLASS, 'It does not support tradingClass parameters in placeOrder')) return;
  }

  if (!_.isEmpty(order.scaleTable) ||
      !_.isEmpty(order.activeStartTime) ||
      !_.isEmpty(order.activeStopTime)) {
    if (this._controller.assertVersion(C.MIN_SERVER_VER.SCALE_TABLE, 'It does not support scaleTable, activeStartTime and activeStopTime parameters')) return;
  }
...
Outgoing.prototype.replaceFA = function (faDataType, xml) {
  if (this._controller.assertVersion(13, 'This feature is not available for this versions of TWS')) return;
...
Outgoing.prototype.reqAccountSummary = function (reqId, group, tags) {
  if (this._controller.assertVersion(C.MIN_SERVER_VER.ACCT_SUMMARY, 'It does not support account summary requests')) return;
...
Outgoing.prototype.reqHeadTimestamp = function(reqId, contract, whatToShow, useRTH, formatDate) {
  ...
  var args = [C.OUTGOING.REQ_HEAD_TIMESTAMP,
              reqId,
              contract.conId,
              contract.symbol,
              contract.secType,
...
Outgoing.prototype.reqContractDetails = function (reqId, contract) {
  if (this._controller.assertVersion(4, 'This feature is not available for this versions of TWS')) return;
  ...
  if (!_.isEmpty(contract.tradingClass)) {
    if (this._controller.assertVersion(C.MIN_SERVER_VER.TRADING_CLASS, 'It does not support tradingClass parameter in reqContractDetails')) return;
  }
...
Outgoing.prototype.reqCurrentTime = function () {
  if (this._controller.assertVersion(33, 'It does not support current time requests')) return;
...
Outgoing.prototype.reqFundamentalData = function (reqId, contract, reportType) {
  ...
  if (contract.conId > 0) {
    if (this._controller.assertVersion(C.MIN_SERVER_VER.TRADING_CLASS, 'It does not support conId parameter in reqFundamentalData')) return;
  }
...
Outgoing.prototype.reqGlobalCancel = function () {
  if (this._controller.assertVersion(C.MIN_SERVER_VER.REQ_GLOBAL_CANCEL, 'It does not support globalCancel requests')) return;
...
Outgoing.prototype.reqHistoricalData = function (tickerId, contract, endDateTime, durationStr,
  ...
  if (this._controller.assertVersion(16, 'It does not support historical data backfill')) return;

  if (!_.isEmpty(contract.tradingClass) || contract.conId > 0) {
    if (this._controller.assertVersion(C.MIN_SERVER_VER.TRADING_CLASS, 'It does not support conId and tradingClass parameters in reqHistroricalData')) return;
  }
...
Outgoing.prototype.reqHistoricalTicks = function (tickerId, contract, startDateTime, endDateTime,
  ...
  if (this._controller.assertVersion(73, 'It does not support historical tick data backfill')) return;
...
Outgoing.prototype.reqTickByTickData = function (tickerId, contract, tickType, numberOfTicks, ignoreSize){
  if (this._controller.assertVersion(73, 'It does not support tick by tick data')) return;
...
Outgoing.prototype.reqMarketDataType = function (marketDataType) {
  if (this._controller.assertVersion(C.MIN_SERVER_VER.REQ_MARKET_DATA_TYPE, 'It does not support marketDataType requests')) return;
...
Outgoing.prototype.reqMktData = function (tickerId, contract, genericTickList, snapshot, regulatorySnapshot) {
  if (snapshot) {
    if (this._controller.assertVersion(C.MIN_SERVER_VER.SNAPSHOT_MKT_DATA, 'It does not support snapshot market data requests')) return;
  }

  if (this._controller.assertVersion(C.MIN_SERVER_VER.UNDER_COMP, 'It does not support delta-neutral orders')) return;

  if (this._controller.assertVersion(C.MIN_SERVER_VER.REQ_MKT_DATA_CONID, 'It does not support conId parameter.')) return;

  if (!_.isEmpty(contract.tradingClass)) {
    if (this._controller.assertVersion(C.MIN_SERVER_VER.TRADING_CLASS, 'It does not support tradingClass parameter in reqMarketData.')) return;
  }
...
Outgoing.prototype.reqMktDepth = function (tickerId, contract, numRows) {
  ...
  if (!_.isEmpty(contract.tradingClass) || contract.conId > 0) {
    if (this._controller.assertVersion(C.MIN_SERVER_VER.TRADING_CLASS, 'It does not support conId and tradingClass parameters in reqMktDepth')) return;
  }
...
Outgoing.prototype.reqPositions = function () {
  if (this._controller.assertVersion(C.MIN_SERVER_VER.ACCT_SUMMARY, 'It does not support position requests')) return;
...
Outgoing.prototype.reqRealTimeBars = function (tickerId, contract, barSize, whatToShow, useRTH) {
  if (this._controller.assertVersion(C.MIN_SERVER_VER.REAL_TIME_BAR, 'It does not support real time bars')) return;

  if (!_.isEmpty(contract.tradingClass) || contract.conId > 0) {
    if (this._controller.assertVersion(C.MIN_SERVER_VER.TRADING_CLASS, 'It does not support conId and tradingClass parameters in reqRealTimeBars')) return;
  }
